...
largeShirts = runners.filter(runner => {
   return runner.shirt_size.toLowerCase().indexOf(('L').toLowerCase()) > -1;
});
...
ticketPriceTotal = runners.reduce((total, currentValue) => total + currentValue.donation, 0);
...
// Problem 1
// display name of althete's working withn skinix and skinte

let workPlace = [];

 runners.forEach(runner => {
   if((runner.company_name === 'Skinix') || (runner.company_name === 'Skinte')) {
 workPlace.push(`Name: ${runner.first_name} ${runner.last_name}, company: ${runner.company_name}`);
   }
})
// console.log(workPlace);

// Problem 2
// display athletes that have made donations higher than 200;
let higherDonations = [];
higherDonations = runners.filter(runner => (runner.donation > 200));
console.log(higherDonations);
// Problem 3
